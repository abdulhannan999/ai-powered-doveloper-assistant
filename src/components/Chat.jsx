...
useEffect(() => {
  ...
  const initializeChat = async () => {
    ...
    const userMessagesQuery = query(
      messagesRef,
      where("userId", "==", user.uid), // ✅ Fixed `where` condition
    ...
    if (snapshot.empty) {
      await addDoc(messagesRef, {
        ...
        role: "ai",
        user: user.uid,
        timestamp: serverTimestamp(), // ✅ Fixed key casing
        isWelcomeMessage: true,
      });
    }
  };

  initializeChat(); // ✅ Added `await` if needed
  ...
  const unsubscribe = onSnapshot(q, (snapshot) => {
    const messageData = snapshot.docs
      ...
      .sort((a, b) => a.timestamp?.toMillis() - b.timestamp?.toMillis()); // Sort in ascending order
  ...
  return () => unsubscribe && unsubscribe(); // ✅ Fixed cleanup
}, [user]);
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const response = results.response?.text?.() || results.response?.content; // ✅ Fixed response handling
